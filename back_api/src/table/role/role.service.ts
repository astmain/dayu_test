...
// import { Role } from '@/prisma/client/postgresql/index';
// interface RoleDTO {
//   name: string;
//   remark?: string;
//   status?: boolean;
  ...
  async getRoleMenuWithPermissionOfAdmin() {
    try {
      ...
      const result = roleWithMenusAndPermissions.map(menu => {
        ...
        const permissionNames = permissionList.map(p => p.value);
  ...
  async getRoleMenuWithPermission2display(id: number) {
    ...
    try {
      ...
      const result = roleWithMenusAndPermissions.menus.map(menu => {
        const permissionNames = menu.permissionList.filter(p => p.roles.length > 0).map(p => p.value);
