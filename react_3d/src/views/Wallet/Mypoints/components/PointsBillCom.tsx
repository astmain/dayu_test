...
import PointsDetailsComLqh from "./PointsDetailsCom"

function PointsBillComLqh() {
  // 账单详情弹框
  ...
  const generateFakeData = (start: number, count: number) => {
    return Array.from({ length: count }, (_, index) => ({
      id: start + index + 1,
      imgSrc: imgUrl + "yuerzhifu2.png",
      title: `3D打印订单支付-余额支付${start + index + 1}`,
  ...
  return (
    <div>
      ...
      <ModalLqh2
        ...
        closable={true}
        content={<PointsDetailsComLqh />}
      />
    </div>
  )
}

export default PointsBillComLqh
