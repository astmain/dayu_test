...
// 引入对公转账弹框
import CorporateTransferCom from "./components/CorporatetransferCom"
// 引入扫码支付弹框
import ScanCodePaymentCom from "./components/ScanCodePaymentCom"

function BalancerechargeComLqh() {
  // 对公转账弹框
  const [isModalOpenDg, setIsModalOpenDg] = useState(false)

  // 扫码支付弹框 isModalOpenCode
  const [isModalOpenCode, setIsModalOpenCode] = useState(false)
  const corporateTransferBtn = () => {
    // 关闭的时候先保留住余额充值对话框,后续调用接口的时候再调整
  ...
  return (
    <div className="h-[42.438vh]  2xl:w-[830px] xl:w-[54vw] lg:w-[52vw] md:w-[50vw] sm:w-[48vw]">
      <div className="text-center mt-[59px] mb-[105px]">
        <span className="text-[20px] text-[#222222]">账户余额：</span>
      ...
      <ModalLqh2
        ...
        closable={true} //显示右上角关闭按钮 />
        content={<ScanCodePaymentCom />}
      />
    </div>
  )
}

export default BalancerechargeComLqh
