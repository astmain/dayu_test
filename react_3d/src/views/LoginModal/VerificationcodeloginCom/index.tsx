import "./VerificationcodeloginComphonenum.css"
...
function VerificationcodeloginComLqh() {
  ...
  const onFinish = (values: any) => {
    ...
    console.log(params, "params传验证码表单参数给后端")
  }
  ...
  const handleChange = (e: any) => {
    ...
    form.setFieldValue("phone", e.target.value.replaceAll(" ", ""))

    // 而第二种方法,一样的代码，但是就是在按下空格的时候，没有提示错误
    // 这个函数的调用，会跳过表单的校验
    // form.setFieldsValue({ phone: e.target.value.replaceAll(" ", "") })
  }

  // 验证码登录--验证码 hangchangeVerification
  const hangchangeVerification = (e: any) => {
    console.log(e.target.value, "e")
    // // 去除空格
    // 这边相同的方法第一种方法:可以把空格替换掉，去除空格，有提示错误
  ...
  return (
    <div className={loginModal.loginyanzhengmahead}>
      <Form
        name="basic"
        onFinish={onFinish}
      ...
      >
        ...
        >
          <Input
            ...
            maxLength={6}
            placeholder="请输入验证码"
            onChange={hangchangeVerification}
            suffix={
        ...
        <Form.Item>
          <div style={{ marginTop: "16px" }} className={loginModal.btnlogin}>
            <Button
